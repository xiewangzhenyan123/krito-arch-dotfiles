...
if [ -f "$WALLPAPER_DIR/$SELECTED_WALLPAPER.jpeg" ]; then
    ...
    sed -i -e "s|preload = .*|preload = $WALLPAPER_DIR/$SELECTED_WALLPAPER.jpeg|" \
           -e "s|wallpaper = ,.*|wallpaper = ,$WALLPAPER_DIR/$SELECTED_WALLPAPER.jpeg|" "$TARGET_FILE"
    sed -i -e "s|path = .*|path = $WALLPAPER_DIR/$SELECTED_WALLPAPER.jpeg|" "$TARGET_FILE2"
